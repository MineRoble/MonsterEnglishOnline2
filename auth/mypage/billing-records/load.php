...
if( isset($_POST['sortKey']) && isset($_POST['sortOrder']) && isset($_POST['itemPerPage']) && isset($_POST['page']) ) {
    ...
    if( $sortKey == "sortby_approvedAt" ) $sortKey = "approved_at";
...
try {
    ...
    if ($count === false) {
        ...
        foreach($result as $item) {
            ...
            // $stmt->bindValue(':user_idx', $_SESSION["user_idx"]);
            // $stmt->execute();
            // $refundable = $stmt->fetchColumn();
            $stmt = $pdo->prepare("SELECT date, start_time FROM schedule WHERE orderId = :orderId AND user_idx = :user_idx AND isCancelled = 0");
            $stmt->bindValue(':orderId', $item["orderId"]);
            $stmt->bindValue(':user_idx', $_SESSION["user_idx"]);
            $stmt->execute();
            $refundableClasses = $stmt->fetchAll(PDO::FETCH_ASSOC);
            $refundableClassCnt = 0;
            foreach($refundableClasses as $refundableClass) {
                if( ( strtotime($refundableClass["date"]." ".$refundableClass["start_time"]) - strtotime(date("Ymd H:i:s")) ) / 3600 > 12 ) $refundableClassCnt++;
            }
            ...
            if($item["method"] == "쿠폰") {
                ...
                array_push($response["body"]["billing-records"], array(
                    ...
                    "refundableClassCnt" => $refundableClassCnt
                ));
